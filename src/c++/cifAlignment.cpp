...
int main(int argc, char** argv)
{
    ...
    struct option long_options[] = {
        ...
        { "outputCif", no_argument, 0, 'f' },
        { "outputAln", no_argument, 0, 'a' },
        { "outputRmsd", no_argument, 0, 'r' },
        { "outputRemoved", no_argument, 0, 'u' },
        { "numReferences", required_argument, 0, 'n' },
        { "continentSize", required_argument, 0, 'x' },
        { "isolationDistance", required_argument, 0, 'y' },
        ...
        case 'o':
            ...
            if (stat(outputDir.c_str(), &info) != 0) {
                ...
                if (mkdir(outputDir.c_str(), 0777) != 0) {
                    std::cerr << "Error: Unable to create directory " << outputDir << '\n';
                    exit(1);
                }
            }
            break;
    ...
    for (size_t iSeq = 0; iSeq < names.size();) {
        ...
        if (!fileExists(struct_file)) {
            // pdb redo filename inclue _final
        ...
        if (nores) {
            ...
            for (gemmi::Model& mod : tst.models) {
                for (gemmi::Residue& res : mod.chains[0].residues) {
                    if (res.seqid.num.value > i) {
                        res.seqid.num.value = res.seqid.num.value - 1;
                    }
                }
            }
        }
    ...
    for (int i = 0; i < len_seq; ++i) {
        ...
        for (auto& entry : freq_map) {
            ...
            if (count > max_count) {
                max_count = count;
                max_aa = aa;
            } else if (count == max_count && aa != max_aa) {
    ...
    for (int ref = 0; ref < nb_ref; ref++) {
        if (ref == 0) {
            if (referenceName.empty()) {
                ...
                if (it != names.end()) {
                    ...
                    std::cerr << "Reference name not found. Using automatic choice." << std::endl;
                    maxarg = getMaxScoreSequence(names, seqs, consensus, len_seq, blosum62);
                }
            }
        ...
        if (ref == 0) {
            ...
            bool sequencesRemoved = removeDuplicates(names, seqs, tst, rmsd_mat, similarity_threshold, output_removed, refName, len_seq, outputDir);
        ...
        if (output_pdb) {
            ...
            gemmi::Ofstream osPDB(outputDir + refName + "_" + names[0] + "_mm.pdb", &std::cout);
        ...
        if (output_rmsd) {
            ...
            std::string rmsdFile = outputDir + refName + "_" + names[0] + "_rmsd.txt";
        ...
        if (output_aln) {
            ...
            std::string alnFile = outputDir + refName + "_" + names[0] + "_aln.fa";
        ...
        if (output_raw_coords) {
            if (!Calpha) {
                ...
                saveTensor(at_rc, outputDir + refName + "_" + names[0] + "_raw_coords.bin");
                saveTensorBool(atom_mask, outputDir + refName + "_" + names[0] + "_raw_coords_mask.bin");
            }
                ...
                void saveTensor(const Tensor3D& tensor, const std::string& filename);
                void saveTensorBool(const Tensor2D_bool& tensor, const std::string& filename);
                saveTensor(at_rc, outputDir + refName + "_" + names[0] + "_raw_coords_ca.bin");
                saveTensorBool(atom_mask, outputDir + refName + "_" + names[0] + "_raw_coords_ca_mask.bin");
            }
        }
        // stats here
    }
            ...
            if (!isContinent) {
                for (const auto& continent : sections) {
                    if (continent.second - continent.first >= continentSize) {
                        ...
                        if (distanceStart <= isolationDistance || distanceEnd <= isolationDistance || distanceToContinentStart <= isolationDistance || distanceToContinentEnd <= isolationDistance) {
                            isIsolated = false;
                            break;
                        }
                    }
            ...
            for (int k = 0; k < len_seq; k++) {
                ...
                if (aligned) {
                    ...
                    if (Calpha) {
                        ...
                        for (const auto& a : tst.models[iSeq].chains[0].residues[indexi].atoms)
                            posi.push_back(a.pos);
                        for (const auto& a : tst.models[jSeq].chains[0].residues[indexj].atoms)
                            posj.push_back(a.pos);
                    }
                }
            }
    ...
    const std::string& outputDir)
{
    ...
    if (output_removed) {
        std::string remfilen = outputDir + refName + "_" + names[0] + "_duplicates.csv";
        if (!isFirstCall) {
