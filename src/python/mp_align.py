...
def launchMafft(args):
    try:
        ...
        with open(cluster_aligned, 'w') as outfile:
            subprocess.run([mafft_path, "--quiet", "--auto", "--amino", "--preservecase", cluster_unaligned], stdout=outfile)
...
def launch_mafft_on_clusters(lclusters_unaligned, name_dr, num_workers=None, mafft_path=None):
    ...
    mafft_args = [(cluster_unaligned, cluster_aligned, mafft_path) for cluster_unaligned, cluster_aligned in zip(lclusters_unaligned, lclusters_aligned)]

    try:
        with multiprocessing.Pool(processes=num_workers) as pool:
            list(tqdm.tqdm(pool.imap_unordered(launchMafft, mafft_args), total=len(lclusters_aligned)))
    except Exception as e:
...
def main():
    ...
    parser.add_argument('-m', '--mafft_path', required=True, help="Path to the MAFFT binary")
    parser.add_argument('-f', '--fasta', required=True, help="Path to the multifasta file")
