...
def apply_mean_coord_on_missing_data(coord_mat, K_mat):
    coord_mat = coord_mat + (
        np.ones(np.shape(coord_mat))
        * (np.sum(coord_mat, axis=1) / np.sum(K_mat, axis=1))[:, np.newaxis]
        * (np.ones(np.shape(K_mat)) - K_mat)
    )
    return coord_mat
...
def computeSumOfPairs(aln, match=1, mismatch=-0.5, gap=-0.5):
    ...
    for j in range(nb_pos):
        ...
        if tgap > 0:
            ...
            Cgaps = 0
        score = (
            score + Cmatch * match + Cgaps * gap + (Ctot - Cgaps - Cmatch) * mismatch
        )
...
def compute_stats(idclu, directory, use_weights=False):
    try:
        ...
        ref_rmsd_max, ref_rmsd_mean, ref_rmsd_std = (
            f"{np.nanmax(ref_rmsd_mat):.3f}",
            f"{np.nanmean(ref_rmsd_mat):.3f}",
            f"{np.nanstd(ref_rmsd_mat):.3f}",
        )
        mat = mat[np.triu_indices(len(mat), k=1)]
        rmsd_max, rmsd_mean, rmsd_std = (
            f"{np.nanmax(mat):.3f}",
            f"{np.nanmean(mat):.3f}",
            f"{np.nanstd(mat):.3f}",
        )
        ...
        col_to_keep = np.where(np.sum(mask, axis=1) > 1)[
            0
        ]  # all the columns with 2 or more residues
        mask = mask[col_to_keep]
        coords = coords[col_to_keep]

        if use_weights:

            eigval_weights_query, eigvec_weights_query = eigss_svd(
                get_query(coords, mask, 0, coverage=True)
            )
            eigval_weights_mean, eigvec_weights_mean = eigss_svd(
                get_mean(coords, mask, coverage=True)
            )
            ...
            eigval_weights_query_norm, eigvec_weights_query_norm = eigss_svd(
                get_query(coords, mask, 0, coverage=True, normalize=True)
            )
            eigval_weights_mean_norm, eigvec_weights_mean_norm = eigss_svd(
                get_mean(coords, mask, coverage=True, normalize=True)
            )
            cumsum_weights_query_norm = np.cumsum(eigval_weights_query_norm)
            cumsum_weights_mean_norm = np.cumsum(eigval_weights_mean_norm)
            nb_modes_weights_query_norm = (
                np.argmax(cumsum_weights_query_norm >= 0.95) + 1
            )
            nb_modes_weights_mean_norm = np.argmax(cumsum_weights_mean_norm >= 0.95) + 1
            comp_pca_weights_query_norm = eigvec_weights_query_norm[
                :, 0:nb_modes_weights_query_norm
            ]
            comp_pca_weights_mean_norm = eigvec_weights_mean_norm[
                :, 0:nb_modes_weights_mean_norm
            ]
            ...
            with open(directory + idclu + "_var_col_weights.csv", "w") as f:
                ...
                for valm, colvalm, valq, colvalq in zip(
                    eigval_weights_mean,
                    col_pca_weights_mean,
                    eigval_weights_query,
                    col_pca_weights_query,
                ):
                    f.write(
                        f"{valm:.5f}"
                        + ","
            ...
            with open(directory + idclu + "_var_col_weights_norm.csv", "w") as f:
                f.write("%var_mean,col_pca_mean,%var_query,col_pca_query\n")
                for valm, colvalm, valq, colvalq in zip(
                    eigval_weights_mean_norm,
                    col_pca_weights_mean_norm,
                    eigval_weights_query_norm,
                    col_pca_weights_query_norm,
                ):
                    f.write(
                        f"{valm:.5f}"
                        + ","
        ...
        if not use_weights:
            ...
            eigval_query_norm, eigvec_query_norm = eigss_svd(
                get_query(coords, mask, 0, normalize=True)
            )
            eigval_mean_norm, eigvec_mean_norm = eigss_svd(
                get_mean(coords, mask, normalize=True)
            )
            ...
            with open(directory + idclu + "_var_col.csv", "w") as f:
                f.write("%var_mean,col_pca_mean,%var_query,col_pca_query\n")
                for valm, colvalm, valq, colvalq in zip(
                    eigval_mean, col_pca_mean, eigval_query, col_pca_query
                ):
                    f.write(
                        f"{valm:.5f}"
                        + ","
...
def main(use_weights, directory, num_workers=None):
    ...
    compute_stats_configured = partial(
        compute_stats, directory=directory, use_weights=use_weights
    )
    ...
    with multiprocessing.Pool(processes=num_workers) as pool:
        results = list(
            tqdm.tqdm(
                pool.imap_unordered(compute_stats_configured, liste_id),
                total=len(liste_id),
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--use_weights",
        action="store_true",
        help="Use this option to enable weights.",
    )
    parser.add_argument(
        "-d",
        "--directory",
