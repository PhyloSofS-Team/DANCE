...
def main():
    ...
    cif_dir = os.path.join(
        script_directory, "test/cifs/"
    )  # Directory containing CIF files
    mmseqs_dir = os.path.join(
        script_directory, "test/mmseqs_output/"
    )  # Directory to store the MMseqs2 output files
    aligned_dir = os.path.join(
        script_directory, "test/aligned/"
    )  # Directory to store the aligned multifasta files of the ensembles before model building
    models_dir = os.path.join(
        script_directory, "test/models/"
    )  # Directory to store the 3D models
    mf_name = os.path.join(
        script_directory, "test/output.fa"
    )  # Name of the multifasta file containing all the sequences extracted from the CIF files of cif_dir
    ...
    mmseqs.process_mmseqs(
        id=identity,
        cov=coverage,
        multifasta=mf_name,
        output_dir=mmseqs_dir,
        num_workers=num_workers,
    )

    # Align sequences of the ensembles with Mafft
    mp_align.process_files(
        multifasta=mf_name,
        tsv=f"{mmseqs_dir}{cluster_db_filename}",
        name_dr=aligned_dir,
        num_workers=num_workers,
    )

    # Run CIF alignment with specified options to create the 3d models
    mp_cifAlignment.run_cif_alignment(
        mfdir=aligned_dir, options=cif_alignment_options, num_workers=num_workers
    )

    # Compute statistics of the models
    mp_write_stats.main(
        use_weights=bool_weight, directory=models_dir, num_workers=num_workers
    )  # to write the stats, the options a, r and b must be enabled in cif_alignment_options
