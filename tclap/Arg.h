...
inline std::wstring Arg::getWDescription() const
{
	std::wstring desc = L"";
	if (_required) {
		wchar_t *wtmp = new wchar_t [_requireLabel.size()];
		mbstowcs(wtmp, _requireLabel.c_str(), _requireLabel.size());
		//(_requireLabel.begin(), _requiredLabel.end());
		std::wstring wstr(wtmp);
		desc = L"(" + wstr + L")  ";
		delete wtmp;
	}
